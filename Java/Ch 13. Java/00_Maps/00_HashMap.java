...
The HashMap class is outstanding for insertions, removals,
...
The rest of this chapter is devoted to showing how, through the miracle of hashing, we can achieve searches—and insertions and removals—in constant time, on average.
...
Hashing is the process of transforming a key into an index in a table. One component of this transformation is the key class’s hashCode() method.

The hashCode() method in the Object class will most likely return the reference itself, that is, the machine address, as an int . Then the hashCode() method applied to two equivalent objects would return different int values!
...
than table.length, so the resulting indexes can span the entire table.
...
One of the requirements of the HashMap class is that table.length must be a power of 2. (This requirement is specific to the Java Collections Framework, and does not apply generally.) For example, if
table.length = 1024 = 2^10, then the index returned by
hash (key.hashCode()) & table.length - 1
...
The advantage of the former expression is that its calculation is quite a bit faster than for the latter expression.
...
Hashing is most efficient when the values of index are spread throughout the table. This notion is referred to as the Uniform Hashing Assumption. Probabilistically speaking, the Uniform Hashing Assumption states that the set of all possible keys is uniformly distributed over the set of all table indexes. That is, each key is equally likely to hash to any one of the table indexes. No class’s hashCode method will satisfy the Uniform Hashing Assumption for all applications, although the supplemental scrambling in the hash function makes it extremely likely (but not guaranteed) that the assumption will hold.
...
by a simple but quite effective technique called “chaining”. Sectio

Chaining
To resolve collisions, we store at each table location the singly-linked list of all elements whose keys have hashed to that index in table. This design is called chained hashing because the elements in each list form a chain.
...
There are two additional fields relating to this discussion: loadFactor (how large can the ratio of size to table length get before resizing will occur) and threshold (how large can size get before resizing will occur).
...
As indicated, the Java Collections Framework’s implementation of hashing uses chaining to handle collisions.
...
It will simplify the implementation and efficiency of the containsKey, put, and remove methods if the table always has at least one open (that is, empty) location. So we require that the load factor be strictly less than 1.0. Recall that with chaining, the load factor can exceed 1.0.

The remove Method

we want to remove the entry with key 033-52-0048 from the table in Figure 14.7. If we simply make that entry null, we will get the table in Figure 14.8. Do you see the pitfall with this removal strategy? The path taken by synonyms of 033-52-0048 has been blocked. A search for the entry with key 214-30-3495 would be unsuccessful, even though there is such an entry in the table.
Instead of nulling out a removed entry, we will add another field to the Entry class:
boolean markedForRemoval;
This field is initialized to false when an entry is inserted into the table. The remove method sets this field to true . The markedForRemoval field, when true , indicates that its entry is no longer part of the hash map, but allows the offset-of-1 collision handler to continue along its path. Note that an entry’s key is examined only if that entry is not marked for removal. The containsKey method loops until an empty or matching entry is found. As with the remove method, an entry’s key is checked only if that entry is not marked for removal. The definition of the containsKey method is only slightly revised from the chained-hashing version. For example, here we use modular arithmetic instead of the & operator because the table length need not be a power of 2.
